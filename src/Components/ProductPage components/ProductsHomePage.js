...
const ProductHomePage = () => {
	...
	const handleViewProducts = () => {
		fetchProducts(); // Call the API to fetch products
	};
	...
	return (
		...
		>
			...
			<div className="absolute top-4 right-4 flex space-x-4">
				<button className="w-12 h-12 bg-white text-black rounded-full flex items-center justify-center hover:bg-gray-500 transition duration-300 text-xl">
					⌕
				</button>
				<button className="w-12 h-12 bg-white text-black rounded-full flex items-center justify-center hover:bg-gray-500 transition duration-300 text-xl">
					↓↑
			...
			{products.length > 0 && (
				<div className="absolute bottom-10 left-1/2 transform -translate-x-1/2 bg-white p-4 rounded-lg shadow-lg">
