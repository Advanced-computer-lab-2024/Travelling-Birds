...
const FilterSection = ({ onFilter }) => {
    const [activeFilter, setActiveFilter] = useState(''); // Track the active filter section
    ...
    return (
        <div className="p-6 bg-white rounded-lg shadow-md max-w-3xl mx-auto">
            ...
            {/* Filter Section Selection Buttons */}
            <div className="mb-6 flex justify-center space-x-6">
                <button 
                    onClick={() => setActiveFilter('activities')}
                    className={`text-gray-700 font-semibold px-4 py-2 rounded-full ${activeFilter === 'activities' ? 'bg-blue-500 text-white' : 'bg-gray-200'}`}
                >
                    Activity Filters
                </button>
                <button 
                    onClick={() => setActiveFilter('itineraries')}
                    className={`text-gray-700 font-semibold px-4 py-2 rounded-full ${activeFilter === 'itineraries' ? 'bg-green-500 text-white' : 'bg-gray-200'}`}
                >
                    Itinerary Filters
                </button>
                <button 
                    onClick={() => setActiveFilter('places')}
                    className={`text-gray-700 font-semibold px-4 py-2 rounded-full ${activeFilter === 'places' ? 'bg-yellow-500 text-white' : 'bg-gray-200'}`}
                >
                    Places
                </button>
            </div>
            ...
            <div className="space-y-4">
                ...
                {activeFilter === 'places' && (
                    <div>
                        <h3 className="text-lg font-semibold text-gray-600 mb-2">Places Filters</h3>
                        <div className="mb-4">
                            <input 
                                type="text" 
