...
const TouristProfile = ({ user, displayOnly }) => {
	...
	const requestAccountDeletion = async () => {
		try {
			const response = await fetch(`${process.env.REACT_APP_BACKEND}/api/users/${user._id}/requestDeletion`, {
				method: 'PATCH',
				headers: { 'Content-Type': 'application/json' },
			});
			...
			if (!response.ok) {
				...
				toast.error(errorData.message || 'Failed to submit deletion request');
				return;
			}
			...
			if (data?.message === 'Deletion request submitted') {
