	...
	return (
		<div
			...
			{showProfileDetails && (
				...
				>
					<h1 className="text-2xl sm:text-3xl font-semibold text-center text-gray-800 mb-4 sm:mb-6">Advertiser
						Profile</h1>
					...
					<div className="flex justify-between">
						<button type="submit"
						        className={`w-full py-2 sm:py-3 mb-3 mr-2 rounded-lg font-semibold ${isEditing ? "bg-green-500 hover:bg-green-600" : "bg-blue-500 hover:bg-blue-600"} text-white transition duration-300`}>
							{isEditing ? 'Confirm' : 'Edit'}
						</button>

						<button type="button" onClick={() => setShowProfileDetails(false)}
					...
					{displayOnly && !user.isApproved && (
						<button type="button" onClick={approveAdvertiser}
						        className="w-full py-2 sm:py-3 mb-3 rounded-lg font-semibold bg-blue-500 hover:bg-blue-600 text-white transition duration-300">
							Approve User
						</button>
					)}

					<button type="button" onClick={() => {
						if (window.confirm('Are you sure you wish to delete this profile?')) requestAccountDeletion();
					}}
