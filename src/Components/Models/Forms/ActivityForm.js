...
const ActivityForm = ({ activity }) => {
    ...
    const registerActivity = () => {
        ...
        formData.append('lwBound', lwBound);
        formData.append('hiBound', hiBound);
    ...
    return (
        <div>
            <form className="w-full max-w-sm mx-auto" onSubmit={(e) => {
                ...
                <input type="file" name="picture" onChange={handleImageChange} className="mt-4" />
                <button type="submit" className="w-full bg-blue-500 text-white py-2 rounded mt-4">{!activity ? 'Register' : 'Update'}</button>
            </form>
        </div>
    );
};
