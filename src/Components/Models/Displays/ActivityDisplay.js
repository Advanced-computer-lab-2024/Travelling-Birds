...
const ActivityDisplay = ({activity}) => {
	...
	const renderRatingStars = (rating) => {
		...
		for (let i = 1; i <= 5; i++) {
			stars.push(
				i <= rating ? <FaStar key={i} className="text-[#330577]"/> :
					<FaRegStar key={i} className="text-[#330577]"/>
			);
		}
		return stars;
	};
			...
			<div className="p-4">
				<div className="text-sm mb-2">
					{activity.tags?.map((tag) => (
						<span key={tag}
						      className="inline-block bg-gray-300 text-gray-900 rounded-full px-2 py-1 mr-1 mt-1">{tag}</span>
					))}
				</div>
